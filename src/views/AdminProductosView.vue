...
onMounted(async () => {
  try {
    await Promise.all([store.loadAllProductos(), store.loadCategorias()])
  } catch {
...
const productos = computed(() => store.getAllProductos())
...
<template>
  <div class="container">
    ...
    <div v-else-if="productos.length > 0" class="table-container">
      <table>
        <thead>
          <tr>
        ...
        <tbody>
          <tr v-for="p in productos" :key="p.id">
            ...
            <td>
              <span class="badge" :class="p.stock < 20 ? 'badge-stock low' : 'badge-stock'">{{ p.stock }} uds</span>
            </td>
            <td><span class="badge badge-cat">{{ getCategoriaNombre(p.idCategoria) }}</span></td>
            <td>
              <div class="actions">
                <button class="btn-icon btn-edit" title="Editar" @click="openEdit(p)"><i class="fas fa-pen"></i></button>
                <button class="btn-icon btn-delete" title="Eliminar" @click="handleDelete(p.id)"><i class="fas fa-trash"></i></button>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <div v-else class="empty-state"><i class="fas fa-box-open"></i><p>No hay productos registrados</p></div>
  </div>
...
.form-group input:focus, .form-group select:focus, .form-group textarea:focus { outline: none; border-color: #e91e63; background: #fff; }
