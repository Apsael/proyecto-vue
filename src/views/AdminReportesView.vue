...
onMounted(async () => {
  try {
    await Promise.all([store.loadVentas(), store.loadAllProductos()])
  } catch {
...
const ventasHoy = computed(() => ventas.value.filter(v => v.fechaVenta.startsWith(hoy)))
const ingresosHoy = computed(() => ventasHoy.value.reduce((sum, v) => sum + v.total, 0))

const topProductos = computed(() => {
  const map = new Map<string, { nombre: string; totalVendido: number; ingresos: number }>()
  for (const v of ventas.value) {
    for (const d of v.detalles) {
      const key = d.nombreProducto ?? 'N/A'
      const existing = map.get(key)
      if (existing) {
        existing.totalVendido += d.cantidad
        existing.ingresos += d.subtotal
      } else {
        map.set(key, { nombre: key, totalVendido: d.cantidad, ingresos: d.subtotal })
      }
    }
  }
...
const ventasPorMetodo = computed(() => {
  ...
  for (const v of ventas.value) {
    const existing = map.get(v.metodoPago)
    if (existing) { existing.cantidad++; existing.monto += v.total }
    else { map.set(v.metodoPago, { cantidad: 1, monto: v.total }) }
  }
...
<template>
  <div class="container">
    ...
    <div class="reports-grid">
      <div class="card">
        <h2><i class="fas fa-trophy" style="color:#ff9800"></i> Top Productos</h2>
        ...
        <div v-else class="empty-state"><i class="fas fa-chart-bar"></i><p>Aun no hay datos de ventas</p></div>
      </div>

      <div class="card">
        <h2><i class="fas fa-credit-card" style="color:#2196f3"></i> Ventas por Metodo</h2>
        <div v-if="ventasPorMetodo.length > 0">
          <div v-for="([metodo, data], idx) in ventasPorMetodo" :key="metodo" class="bar-container">
            ...
            <div class="bar-track">
              <div class="bar-fill" :class="barColors[idx % 3]" :style="{ width: (data.monto / maxMetodo) * 100 + '%' }">${{ data.monto.toFixed(2) }}</div>
            </div>
          </div>
        </div>
        <div v-else class="empty-state"><i class="fas fa-credit-card"></i><p>Sin datos aun</p></div>
      </div>
      ...
      <div class="card full-width">
        ...
        <div v-else class="empty-state"><i class="fas fa-check-circle"></i><p>Todos los productos tienen stock suficiente</p></div>
      </div>
    </div>
  </div>
</template>
...
.stat-card { background: white; border-radius: 16px; padding: 24px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.05); position: relative; overflow: hidden; }
...
.stat-icon { width: 50px; height: 50px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 22px; color: white; margin-bottom: 14px; }
...
.card h2 { font-size: 17px; color: #333; margin-bottom: 18px; display: flex; align-items: center; gap: 10px; }
table { width: 100%; border-collapse: collapse; }
thead th { background: #fafafa; padding: 12px 14px; text-align: left; font-size: 12px; font-weight: 600; color: #555; border-bottom: 2px solid #f0f0f0; }
tbody td { padding: 11px 14px; font-size: 13px; color: #444; border-bottom: 1px solid #f5f5f5; }
tbody tr:hover { background: #fdf2f8; }
.price { font-weight: 600; color: #2e7d32; }
.bar-container { display: flex; align-items: center; gap: 12px; margin-bottom: 12px; }
.bar-label { font-size: 13px; color: #555; width: 100px; flex-shrink: 0; }
.bar-track { flex: 1; height: 24px; background: #f0f0f0; border-radius: 12px; overflow: hidden; }
.bar-fill { height: 100%; border-radius: 12px; transition: width 0.5s ease; display: flex; align-items: center; padding-left: 10px; font-size: 11px; font-weight: 600; color: white; }
...
.stock-alert { display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #f5f5f5; }
.stock-alert:last-child { border-bottom: none; }
.stock-alert .name { font-size: 13px; color: #444; }
.stock-alert .qty { font-size: 13px; font-weight: 600; color: #e65100; background: #fff3e0; padding: 3px 10px; border-radius: 10px; }
.empty-state { text-align: center; padding: 35px 20px; color: #aaa; }
.empty-state i { font-size: 36px; margin-bottom: 10px; display: block; }
.full-width { grid-column: 1 / -1; }
@media (max-width: 768px) { .stats-grid { grid-template-columns: repeat(2, 1fr); } .reports-grid { grid-template-columns: 1fr; } }
</style>
